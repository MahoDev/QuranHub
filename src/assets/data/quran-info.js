export const surahNames = {
  1: "سورة الفاتحة",
  2: "سورة البقرة",
  3: "سورة آل عمران",
  4: "سورة النساء",
  5: "سورة المائدة",
  6: "سورة الأنعام",
  7: "سورة الأعراف",
  8: "سورة الأنفال",
  9: "سورة التوبة",
  10: "سورة يونس",
  11: "سورة هود",
  12: "سورة يوسف",
  13: "سورة الرعد",
  14: "سورة إبراهيم",
  15: "سورة الحجر",
  16: "سورة النحل",
  17: "سورة الإسراء",
  18: "سورة الكهف",
  19: "سورة مريم",
  20: "سورة طه",
  21: "سورة الأنبياء",
  22: "سورة الحج",
  23: "سورة المؤمنون",
  24: "سورة النور",
  25: "سورة الفرقان",
  26: "سورة الشعراء",
  27: "سورة النمل",
  28: "سورة القصص",
  29: "سورة العنكبوت",
  30: "سورة الروم",
  31: "سورة لقمان",
  32: "سورة السجدة",
  33: "سورة الأحزاب",
  34: "سورة سبأ",
  35: "سورة فاطر",
  36: "سورة يس",
  37: "سورة الصافات",
  38: "سورة ص",
  39: "سورة الزمر",
  40: "سورة غافر",
  41: "سورة فصلت",
  42: "سورة الشورى",
  43: "سورة الزخرف",
  44: "سورة الدخان",
  45: "سورة الجاثية",
  46: "سورة الأحقاف",
  47: "سورة محمد",
  48: "سورة الفتح",
  49: "سورة الحشر",
  50: "سورة الممتحنة",
  51: "سورة القمر",
  52: "سورة الرحمن",
  53: "سورة الواقعة",
  54: "سورة الحديد",
  55: "سورة المجادلة",
  56: "سورة الحشر",
  57: "سورة الممتحنة",
  58: "سورة القمر",
  59: "سورة الرحمن",
  60: "سورة الواقعة",
  61: "سورة الحديد",
  62: "سورة المجادلة",
  63: "سورة المنافقون",
  64: "سورة التغابن",
  65: "سورة الطلاق",
  66: "سورة التحريم",
  67: "سورة الملك",
  68: "سورة القلم",
  69: "سورة الحاقة",
  70: "سورة المعارج",
  71: "سورة نوح",
  72: "سورة الجن",
  73: "سورة المزمل",
  74: "سورة المدثر",
  75: "سورة القيامة",
  76: "سورة الإنسان",
  77: "سورة المرسلات",
  78: "سورة النبأ",
  79: "سورة النازعات",
  80: "سورة عبس",
  81: "سورة التكوير",
  82: "سورة الإنفطار",
  83: "سورة المطففين",
  84: "سورة الإنشقاق",
  85: "سورة البروج",
  86: "سورة الطارق",
  87: "سورة الأعلى",
  88: "سورة الغاشية",
  89: "سورة الفجر",
  90: "سورة البلد",
  91: "سورة الشمس",
  92: "سورة الليل",
  93: "سورة الضحى",
  94: "سورة الشرح",
  95: "سورة التين",
  96: "سورة العلق",
  97: "سورة القدر",
  98: "سورة البينة",
  99: "سورة الزلزلة",
  100: "سورة العاديات",
  101: "سورة القارعة",
  102: "سورة التكاثر",
  103: "سورة العصر",
  104: "سورة الهمزة",
  105: "سورة الفيل",
  106: "سورة قريش",
  107: "سورة الماعون",
  108: "سورة الكوثر",
  109: "سورة الكافرون",
  110: "سورة النصر",
  111: "سورة المسد",
  112: "سورة الإخلاص",
  113: "سورة الفلق",
  114: "سورة الناس",
};

export const surahNumToPagesMap = {
  1: [1, 1], // Al-Fatiha
  2: [2, 49], // Al-Baqarah
  3: [50, 76], // Al-Imran
  4: [77, 106], // Al-Nisa
  5: [106, 127], // Al-Maida
  6: [128, 150], // Al-An'am
  7: [151, 176], // Al-A'raf
  8: [177, 186], // Al-Anfal
  9: [187, 207], // Al-Tawba
  10: [208, 221], // Yunus
  11: [221, 235], // Hud
  12: [235, 248], // Yusuf
  13: [249, 255], // Al-Ra'd
  14: [255, 261], // Ibrahim
  15: [262, 267], // Al-Hijr
  16: [267, 281], // Al-Nahl
  17: [282, 293], // Al-Israa
  18: [293, 304], // Al-Kahf
  19: [305, 312], // Mariam
  20: [312, 321], // Taha
  21: [322, 331], // Al-Anbia'
  22: [332, 341], // Al-Hajj
  23: [342, 349], // Al-Mu'minun
  24: [350, 359], // Al-Nur
  25: [359, 366], // Al-Furqan
  26: [367, 376], // Ash-Shu'ara
  27: [377, 385], // Al-Naml
  28: [385, 396], // Al-Qasas
  29: [396, 404], // Al-Ankabout
  30: [404, 410], // Al-Rum
  31: [411, 414], // Luqman
  32: [415, 417], // Al-Sajda
  33: [418, 427], // Al-Ahzab
  34: [428, 434], // Saba
  35: [434, 440], // Fatir
  36: [440, 445], // Ya-Sin
  37: [446, 452], // Al-Safat
  38: [453, 458], // Sad
  39: [458, 467], // Al-Zumar
  40: [467, 476], // Ghafir
  41: [477, 482], // Fussilat
  42: [483, 489], // Al-Shuraa
  43: [489, 495], // Al-Zukhruf
  44: [496, 498], // Al-Dukhan
  45: [499, 502], // Al-Jathiyah
  46: [502, 506], // Al-Ahqaf
  47: [507, 510], // Muhammed
  48: [511, 515], // Al-Fath
  49: [515, 517], // Al-Hujrat
  50: [518, 520], // Qaf
  51: [520, 523], // Al-Zariat
  52: [523, 525], // Al-Tur
  53: [526, 528], // Al-Najm
  54: [526, 531], // Al-Qamar
  55: [531, 534], // Al-Rahman
  56: [534, 537], // Al-Waqi'a
  57: [537, 541], // Al-Hadid
  58: [542, 545], // Al-Mugadala
  59: [545, 548], // Al-Hashr
  60: [549, 551], // Al-Mumtahana
  61: [551, 552], // Al-Saff
  62: [553, 554], // Al-Juma'a
  63: [554, 555], // Al-Munafiqun
  64: [556, 557], // Al-Taghabun
  65: [558, 559], // Al-Talaq
  66: [560, 561], // Al-Tahrim
  67: [562, 564], // Al-Mulk
  68: [564, 566], // Al-Qalam
  69: [566, 568], // Al-Haqa
  70: [568, 570], // Al-Ma'arij
  71: [570, 571], // Nuh
  72: [572, 573], // Al-Jin
  73: [574, 575], // Al-Muzammel
  74: [575, 577], // Al-Mudather
  75: [577, 578], // Al-Qiyama
  76: [578, 580], // Al-Insan
  77: [580, 581], // Al-Mursalat
  78: [582, 583], // Al-Naba'
  79: [583, 584], // Al-Nazi'at
  80: [585, 586], // Abs
  81: [586, 586], // Al-Takwer
  82: [587, 587], // Al-Inftar
  83: [587, 589], // Al-Mutaffifin
  84: [589, 590], // Al-Inshqaq
  85: [590, 590], // Al-Buruj
  86: [591, 591], // Al-Tariq
  87: [591, 592], // Al-A'la
  88: [592, 593], // Al-Ghashiyah
  89: [593, 594], // Al-Fajr
  90: [594, 595], // Al-Balad
  91: [595, 595], // Al-Shams
  92: [595, 596], // Al-Layel
  93: [596, 596], // Al-Duha
  94: [596, 597], // Al-Sharh
  95: [597, 597], // Al-Tin
  96: [597, 598], // Al-Alaq
  97: [598, 598], // Al-Qadar
  98: [598, 599], // Al-Bayyinah
  99: [599, 599], // Al-Zalzalah
  100: [599, 600], // Al-Adiyat
  101: [600, 600], // Al-Qari'ah
  102: [600, 600], // Al-Takathur
  103: [601, 601], // Al-Asr
  104: [601, 601], // Al-Humazah
  105: [601, 601], // Al-Fil
  106: [602, 602], // Quraysh
  107: [602, 602], // Al-Ma'un
  108: [602, 602], // Al-Kawthar
  109: [603, 603], // Al-Kafirun
  110: [603, 603], // Al-Nasr
  111: [603, 603], // Al-Masad
  112: [604, 604], // Al-Ikhlas
  113: [604, 604], // Al-Falaq
  114: [604, 604], // Al-Nas
};

export const quranPages = Array.from(Array(604).keys()).map((num) => num + 1);

export const tafseerTypes = {
  1: "التفسير الميسر",
  2: "تفسير الجلالين",
  3: "تفسير السعدي",
  4: "تفسير ابن كثير",
  5: "تفسير الوسيط لطنطاوي",
  6: "تفسير البغوي",
  7: "تفسير القرطبي",
  8: "تفسير الطبري",
};

// export const quranRecitations = [
//   {
//     id: 1,
//     reciter_name: "عبد الباسط عبد الصمد",
//     style: "مجود",
//   },
//   {
//     id: 2,
//     reciter_name: "عبد الباسط عبد الصمد",
//     style: "مرتل",
//   },
//   {
//     id: 3,
//     reciter_name: "عبدالرحمن السديس",
//     style: null,
//   },
//   {
//     id: 4,
//     reciter_name: "أبو بكر الشاطرى",
//     style: null,
//   },
//   {
//     id: 5,
//     reciter_name: "هاني الرفاعي",
//     style: null,
//   },
//   {
//     id: 6,
//     reciter_name: "محمود خليل الحصري",
//     style: null,
//   },
//   {
//     id: 7,
//     reciter_name: "مشاري راشد العفاسي",
//     style: null,
//   },
//   {
//     id: 8,
//     reciter_name: "محمد صديق المنشاوي",
//     style: "مجود",
//   },
//   {
//     id: 9,
//     reciter_name: "محمد صديق المنشاوي",
//     style: "مرتل",
//   },
//   {
//     id: 10,
//     reciter_name: "سعود الشريم",
//     style: null,
//   },
//   {
//     id: 11,
//     reciter_name: "محمد الطبلاوي",
//     style: null,
//   },
//   {
//     id: 12,
//     reciter_name: "محمود خليل الحصري",
//     style: "مُعلِم",
//   },
// ];

// export const quranRecitation = [
//   {
//     1: {
//       subfolder: "Abdul_Basit_Murattal_64kbps",
//       name: "عبدالباسط عبد الصمد (مرتل)",
//       bitrate: "64kbps",
//     },
//     2: {
//       subfolder: "Abdul_Basit_Murattal_192kbps",
//       name: "عبدالباسط عبد الصمد (مرتل)",
//       bitrate: "192kbps",
//     },
//     3: {
//       subfolder: "Abdul_Basit_Mujawwad_128kbps",
//       name: "عبدالباسط عبد الصمد (مجود)",
//       bitrate: "128kbps",
//     },
//     4: {
//       subfolder: "Abdullah_Basfar_32kbps",
//       name: "عبدالله بصفر",
//       bitrate: "32kbps",
//     },
//     5: {
//       subfolder: "Abdullah_Basfar_64kbps",
//       name: "عبدالله بصفر",
//       bitrate: "64kbps",
//     },
//     6: {
//       subfolder: "Abdullah_Basfar_192kbps",
//       name: "عبدالله بصفر",
//       bitrate: "192kbps",
//     },
//     7: {
//       subfolder: "Abdurrahmaan_As-Sudais_64kbps",
//       name: "عبدالرحمن السديس",
//       bitrate: "64kbps",
//     },
//     8: {
//       subfolder: "Abdurrahmaan_As-Sudais_192kbps",
//       name: "عبدالرحمن السديس",
//       bitrate: "192kbps",
//     },
//     9: {
//       subfolder: "AbdulSamad_64kbps_QuranExplorer.Com",
//       name: "عبدالباسط عبد الصمد (مجود)",
//       bitrate: "64kbps",
//     },
//     10: {
//       subfolder: "Abu_Bakr_Ash-Shaatree_64kbps",
//       name: "ابو بكر الشاطري",
//       bitrate: "64kbps",
//     },
//     11: {
//       subfolder: "Abu_Bakr_Ash-Shaatree_128kbps",
//       name: "ابو بكر الشاطري",
//       bitrate: "128kbps",
//     },
//     12: {
//       subfolder: "Ahmed_ibn_Ali_al-Ajamy_64kbps_QuranExplorer.Com",
//       name: "أحمد العجمي",
//       bitrate: "64kbps",
//     },
//     13: {
//       subfolder: "Ahmed_ibn_Ali_al-Ajamy_128kbps_ketaballah.net",
//       name: "أحمد العجمي",
//       bitrate: "128kbps",
//     },
//     14: {
//       subfolder: "Alafasy_64kbps",
//       name: "مشاري العفاسي",
//       bitrate: "64kbps",
//     },
//     15: {
//       subfolder: "Alafasy_128kbps",
//       name: "مشاري العفاسي",
//       bitrate: "128kbps",
//     },
//     16: {
//       subfolder: "Ghamadi_40kbps",
//       name: "سعد الغامدي",
//       bitrate: "40kbps",
//     },
//     17: {
//       subfolder: "Hani_Rifai_64kbps",
//       name: "هاني الرفاعي",
//       bitrate: "64kbps",
//     },
//     18: {
//       subfolder: "Hani_Rifai_192kbps",
//       name: "هاني الرفاعي",
//       bitrate: "192kbps",
//     },
//     19: {
//       subfolder: "Husary_64kbps",
//       name: "محمود خليل الحصري (مرتل)",
//       bitrate: "64kbps",
//     },
//     20: {
//       subfolder: "Husary_128kbps",
//       name: "محمود خليل الحصري (مرتل)",
//       bitrate: "128kbps",
//     },
//     21: {
//       subfolder: "Husary_Mujawwad_64kbps",
//       name: "محمود خليل الحصري (مجود)",
//       bitrate: "64kbps",
//     },
//     22: {
//       subfolder: "Husary_128kbps_Mujawwad",
//       name: "محمود خليل الحصري (مجود)",
//       bitrate: "128kbps",
//     },
//     23: {
//       subfolder: "Hudhaify_32kbps",
//       name: "علي الحذيفي",
//       bitrate: "32kbps",
//     },
//     24: {
//       subfolder: "Hudhaify_64kbps",
//       name: "علي الحذيفي",
//       bitrate: "64kbps",
//     },
//     25: {
//       subfolder: "Hudhaify_128kbps",
//       name: "علي الحذيفي",
//       bitrate: "128kbps",
//     },
//     26: {
//       subfolder: "Ibrahim_Akhdar_32kbps",
//       name: "ابراهيم الاخضر",
//       bitrate: "32kbps",
//     },
//     27: {
//       //remove
//       subfolder: "Ibrahim_Akhdar_64kbps",
//       name: "Ibrahim Akhdar",
//       bitrate: "64kbps",
//     },
//     28: {
//       subfolder: "Maher_AlMuaiqly_64kbps",
//       name: "ماهر المعيقلي",
//       bitrate: "64kbps",
//     },
//     29: {
//       subfolder: "MaherAlMuaiqly128kbps",
//       name: "ماهر المعيقلي",
//       bitrate: "128kbps",
//     },
//     30: {
//       subfolder: "Menshawi_16kbps",
//       name: "محمد صديق المنشاوي (مرتل)",
//       bitrate: "16kbps",
//     },
//     31: {
//       //remove
//       subfolder: "Menshawi_32kbps",
//       name: "Menshawi",
//       bitrate: "32kbps",
//     },
//     32: {
//       subfolder: "Minshawy_Mujawwad_64kbps",
//       name: "محمد صديق المنشاوي (مجود)",
//       bitrate: "64kbps",
//     },
//     33: {
//       subfolder: "Minshawy_Mujawwad_192kbps",
//       name: "محمد صديق المنشاوي (مجود)",
//       bitrate: "192kbps",
//     },
//     34: {
//       subfolder: "Minshawy_Murattal_128kbps",
//       name: "محمد صديق المنشاوي (مرتل)",
//       bitrate: "128kbps",
//     },
//     35: {
//       subfolder: "Mohammad_al_Tablaway_64kbps",
//       name: "محمد الطبلاوي (مرتل)",
//       bitrate: "64kbps",
//     },
//     36: {
//       subfolder: "Mohammad_al_Tablaway_128kbps",
//       name: "محمد الطبلاوي (مرتل)",
//       bitrate: "128kbps",
//     },
//     37: {
//       subfolder: "Muhammad_Ayyoub_128kbps",
//       name: "محمد أيوب",
//       bitrate: "128kbps",
//     },
//     38: {
//       subfolder: "Muhammad_Ayyoub_64kbps",
//       name: "محمد أيوب",
//       bitrate: "64kbps",
//     },
//     39: {
//       subfolder: "Muhammad_Ayyoub_32kbps",
//       name: "محمد أيوب",
//       bitrate: "32kbps",
//     },
//     40: {
//       subfolder: "Muhammad_Jibreel_64kbps",
//       name: "محمد جبريل",
//       bitrate: "64kbps",
//     },
//     41: {
//       subfolder: "Muhammad_Jibreel_128kbps",
//       name: "محمد جبريل",
//       bitrate: "128kbps",
//     },
//     42: {
//       //remove
//       subfolder: "Mustafa_Ismail_48kbps",
//       name: "Mustafa Ismail",
//       bitrate: "48kbps",
//     },
//     43: {
//       subfolder: "Saood_ash-Shuraym_64kbps",
//       name: "سعود الشريم",
//       bitrate: "64kbps",
//     },
//     44: {
//       subfolder: "Saood_ash-Shuraym_128kbps",
//       name: "سعود الشريم",
//       bitrate: "128kbps",
//     },
//     45: {
//       //remove
//       subfolder: "English/Sahih_Intnl_Ibrahim_Walk_192kbps",
//       name: "(English) Translated by Sahih International Recited by Ibrahim Walk",
//       bitrate: "192kbps",
//     },
//     46: {
//       //remove
//       subfolder: "MultiLanguage/Basfar_Walk_192kbps",
//       name: "MultiLanguage/Basfar Walk",
//       bitrate: "192kbps",
//     },
//     47: {
//       //remove
//       subfolder: "translations/Makarem_Kabiri_16Kbps",
//       name: "(Persian) Translated by Makarem Recited by Kabiri",
//       bitrate: "64Kbps",
//     },
//     48: {
//       //remove
//       subfolder: "translations/Fooladvand_Hedayatfar_40Kbps",
//       name: "(Persian) Translated by Fooladvand Recited by Hedayatfar",
//       bitrate: "64Kbps",
//     },
//     49: {
//       //remove
//       subfolder: "Parhizgar_48kbps",
//       name: "Parhizgar_64Kbps",
//       bitrate: "64Kbps",
//     },
//     50: {
//       //remove
//       subfolder: "translations/azerbaijani/balayev",
//       name: "Balayev",
//       bitrate: "64Kbps",
//     },
//     51: {
//       subfolder: "Salaah_AbdulRahman_Bukhatir_128kbps",
//       name: "صلاح بوخاطر",
//       bitrate: "128kbps",
//     },
//     52: {
//       subfolder: "Muhsin_Al_Qasim_192kbps",
//       name: "عبدالمحسن القاسم",
//       bitrate: "192kbps",
//     },
//     53: {
//       subfolder: "Abdullaah_3awwaad_Al-Juhaynee_128kbps",
//       name: "عبدالله عواد الجهني",
//       bitrate: "128kbps",
//     },
//     54: {
//       subfolder: "Salah_Al_Budair_128kbps",
//       name: "صلاح البدير",
//       bitrate: "128kbps",
//     },
//     55: {
//       subfolder: "Abdullah_Matroud_128kbps",
//       name: "عبد الله المطرود",
//       bitrate: "128kbps",
//     },
//     56: {
//       subfolder: "Ahmed_Neana_128kbps",
//       name: "أحمد نعينع (مرتل)",
//       bitrate: "128kbps",
//     },
//     57: {
//       subfolder: "Muhammad_AbdulKareem_128kbps",
//       name: "محمد عبد الكريم",
//       bitrate: "128kbps",
//     },
//     58: {
//       subfolder: "khalefa_al_tunaiji_64kbps",
//       name: "خليفة الطنيجي",
//       bitrate: "64kbps",
//     },
//     59: {
//       subfolder: "mahmoud_ali_al_banna_32kbps",
//       name: "محمود علي البنا (مجود)",
//       bitrate: "32kbps",
//     },
//     60: {
//       //delete
//       subfolder: "warsh/warsh_ibrahim_aldosary_128kbps",
//       name: "(Warsh) Ibrahim Al-Dosary",
//       bitrate: "128kbps",
//     },
//     61: {
//       //delete
//       subfolder: "warsh/warsh_yassin_al_jazaery_64kbps",
//       name: "(Warsh) Yassin Al-Jazaery",
//       bitrate: "64kbps",
//     },
//     62: {
//       //delete
//       subfolder: "warsh/warsh_Abdul_Basit_128kbps",
//       name: "(Warsh) Abdul Basit",
//       bitrate: "128kbps",
//     },
//     63: {
//       //delete
//       subfolder: "translations/urdu_shamshad_ali_khan_46kbps",
//       name: "(Urdu) Shamshad Ali Khan",
//       bitrate: "46kbps",
//     },
//     64: {
//       subfolder: "Karim_Mansoori_40kbps",
//       name: "كريم منصوري",
//       bitrate: "40kbps",
//     },
//     65: {
//       subfolder: "Husary_Muallim_128kbps",
//       name: "محمود خليل الحصري (مُعلِم)",
//       bitrate: "128kbps",
//     },
//     66: {
//       subfolder: "Khaalid_Abdullaah_al-Qahtaanee_192kbps",
//       name: "خالد عبدالله القحطاني",
//       bitrate: "192kbps",
//     },
//     67: {
//       subfolder: "Yasser_Ad-Dussary_128kbps",
//       name: "ياسر الدوسري",
//       bitrate: "128kbps",
//     },
//     68: {
//       subfolder: "Nasser_Alqatami_128kbps",
//       name: "ناصر القطامي",
//       bitrate: "128kbps",
//     },
//     69: {
//       subfolder: "Ali_Hajjaj_AlSuesy_128kbps",
//       name: "علي حجاج السويسي",
//       bitrate: "128kbps",
//     },
//     70: {
//       subfolder: "Sahl_Yassin_128kbps",
//       name: "سهل زين يس",
//       bitrate: "128kbps",
//     },
//     71: {
//       subfolder: "ahmed_ibn_ali_al_ajamy_128kbps",
//       name: "احمد العجمي",
//       bitrate: "128kbps",
//     },
//     72: {
//       //delete
//       subfolder: "translations/besim_korkut_ajet_po_ajet",
//       name: "Besim Korkut (Bosnian)",
//       bitrate: "128kbps",
//     },
//     73: {
//       subfolder: "aziz_alili_128kbps",
//       name: "عزيز العليلي",
//       bitrate: "128kbps",
//     },
//     74: {
//       subfolder: "Yaser_Salamah_128kbps",
//       name: "ياسر سلامة",
//       bitrate: "128kbps",
//     },
//     75: {
//       subfolder: "Akram_AlAlaqimy_128kbps",
//       name: "أكرم العلاقمي",
//       bitrate: "128kbps",
//     },
//     76: {
//       subfolder: "Ali_Jaber_64kbps",
//       name: "علي جابر",
//       bitrate: "64kbps",
//     },
//     77: {
//       subfolder: "Fares_Abbad_64kbps",
//       name: "فارس عباد",
//       bitrate: "64kbps",
//     },
//     78: {
//       //delete
//       subfolder: "translations/urdu_farhat_hashmi",
//       name: "Farhat Hashmi (Urdu word for word translation)",
//       bitrate: "32kbps",
//     },
//     79: {
//       subfolder: "Ayman_Sowaid_64kbps",
//       name: "أيمن سويد",
//       bitrate: "64kbps",
//     },
//   },
// ];

//to deal with different qualties I can hide repeating names and then filter
//the objects with the same "name" property, and later display these objects
//in a seperate box to give the option to choose qualties.
export const quranRecitations = {
  1: {
    subfolder: "Abdul_Basit_Murattal_64kbps",
    name: "عبدالباسط عبد الصمد (مرتل)",
    bitrate: "64kbps",
  },
  2: {
    subfolder: "Abdul_Basit_Murattal_192kbps",
    name: "عبدالباسط عبد الصمد (مرتل)",
    bitrate: "192kbps",
  },
  3: {
    subfolder: "Abdul_Basit_Mujawwad_128kbps",
    name: "عبدالباسط عبد الصمد (مجود)",
    bitrate: "128kbps",
  },
  4: {
    subfolder: "Abdullah_Basfar_32kbps",
    name: "عبدالله بصفر",
    bitrate: "32kbps",
  },
  5: {
    subfolder: "Abdullah_Basfar_64kbps",
    name: "عبدالله بصفر",
    bitrate: "64kbps",
  },
  6: {
    subfolder: "Abdullah_Basfar_192kbps",
    name: "عبدالله بصفر",
    bitrate: "192kbps",
  },
  7: {
    subfolder: "Abdurrahmaan_As-Sudais_64kbps",
    name: "عبدالرحمن السديس",
    bitrate: "64kbps",
  },
  8: {
    subfolder: "Abdurrahmaan_As-Sudais_192kbps",
    name: "عبدالرحمن السديس",
    bitrate: "192kbps",
  },
  9: {
    subfolder: "AbdulSamad_64kbps_QuranExplorer.Com",
    name: "عبدالباسط عبد الصمد (مجود)",
    bitrate: "64kbps",
  },
  10: {
    subfolder: "Abu_Bakr_Ash-Shaatree_64kbps",
    name: "ابو بكر الشاطري",
    bitrate: "64kbps",
  },
  11: {
    subfolder: "Abu_Bakr_Ash-Shaatree_128kbps",
    name: "ابو بكر الشاطري",
    bitrate: "128kbps",
  },
  12: {
    subfolder: "Ahmed_ibn_Ali_al-Ajamy_64kbps_QuranExplorer.Com",
    name: "أحمد العجمي",
    bitrate: "64kbps",
  },
  13: {
    subfolder: "Ahmed_ibn_Ali_al-Ajamy_128kbps_ketaballah.net",
    name: "أحمد العجمي",
    bitrate: "128kbps",
  },
  14: {
    subfolder: "Alafasy_64kbps",
    name: "مشاري العفاسي",
    bitrate: "64kbps",
  },
  15: {
    subfolder: "Alafasy_128kbps",
    name: "مشاري العفاسي",
    bitrate: "128kbps",
  },
  16: {
    subfolder: "Ghamadi_40kbps",
    name: "سعد الغامدي",
    bitrate: "40kbps",
  },
  17: {
    subfolder: "Hani_Rifai_64kbps",
    name: "هاني الرفاعي",
    bitrate: "64kbps",
  },
  18: {
    subfolder: "Hani_Rifai_192kbps",
    name: "هاني الرفاعي",
    bitrate: "192kbps",
  },
  19: {
    subfolder: "Husary_64kbps",
    name: "محمود خليل الحصري (مرتل)",
    bitrate: "64kbps",
  },
  20: {
    subfolder: "Husary_128kbps",
    name: "محمود خليل الحصري (مرتل)",
    bitrate: "128kbps",
  },
  21: {
    subfolder: "Husary_Mujawwad_64kbps",
    name: "محمود خليل الحصري (مجود)",
    bitrate: "64kbps",
  },
  22: {
    subfolder: "Husary_128kbps_Mujawwad",
    name: "محمود خليل الحصري (مجود)",
    bitrate: "128kbps",
  },
  23: {
    subfolder: "Hudhaify_32kbps",
    name: "علي الحذيفي",
    bitrate: "32kbps",
  },
  24: {
    subfolder: "Hudhaify_64kbps",
    name: "علي الحذيفي",
    bitrate: "64kbps",
  },
  25: {
    subfolder: "Hudhaify_128kbps",
    name: "علي الحذيفي",
    bitrate: "128kbps",
  },
  26: {
    subfolder: "Ibrahim_Akhdar_32kbps",
    name: "ابراهيم الاخضر",
    bitrate: "32kbps",
  },
  27: {
    subfolder: "Maher_AlMuaiqly_64kbps",
    name: "ماهر المعيقلي",
    bitrate: "64kbps",
  },
  28: {
    subfolder: "MaherAlMuaiqly128kbps",
    name: "ماهر المعيقلي",
    bitrate: "128kbps",
  },
  29: {
    subfolder: "Menshawi_16kbps",
    name: "محمد صديق المنشاوي (مرتل)",
    bitrate: "16kbps",
  },
  30: {
    subfolder: "Minshawy_Mujawwad_64kbps",
    name: "محمد صديق المنشاوي (مجود)",
    bitrate: "64kbps",
  },
  31: {
    subfolder: "Minshawy_Mujawwad_192kbps",
    name: "محمد صديق المنشاوي (مجود)",
    bitrate: "192kbps",
  },
  32: {
    subfolder: "Minshawy_Murattal_128kbps",
    name: "محمد صديق المنشاوي (مرتل)",
    bitrate: "128kbps",
  },
  33: {
    subfolder: "Mohammad_al_Tablaway_64kbps",
    name: "محمد الطبلاوي (مرتل)",
    bitrate: "64kbps",
  },
  34: {
    subfolder: "Mohammad_al_Tablaway_128kbps",
    name: "محمد الطبلاوي (مرتل)",
    bitrate: "128kbps",
  },
  35: {
    subfolder: "Muhammad_Ayyoub_128kbps",
    name: "محمد أيوب",
    bitrate: "128kbps",
  },
  36: {
    subfolder: "Muhammad_Ayyoub_64kbps",
    name: "محمد أيوب",
    bitrate: "64kbps",
  },
  37: {
    subfolder: "Muhammad_Ayyoub_32kbps",
    name: "محمد أيوب",
    bitrate: "32kbps",
  },
  38: {
    subfolder: "Muhammad_Jibreel_64kbps",
    name: "محمد جبريل",
    bitrate: "64kbps",
  },
  39: {
    subfolder: "Muhammad_Jibreel_128kbps",
    name: "محمد جبريل",
    bitrate: "128kbps",
  },
  40: {
    subfolder: "Saood_ash-Shuraym_64kbps",
    name: "سعود الشريم",
    bitrate: "64kbps",
  },
  41: {
    subfolder: "Saood_ash-Shuraym_128kbps",
    name: "سعود الشريم",
    bitrate: "128kbps",
  },
  42: {
    subfolder: "Salaah_AbdulRahman_Bukhatir_128kbps",
    name: "صلاح بوخاطر",
    bitrate: "128kbps",
  },
  43: {
    subfolder: "Muhsin_Al_Qasim_192kbps",
    name: "عبدالمحسن القاسم",
    bitrate: "192kbps",
  },
  44: {
    subfolder: "Abdullaah_3awwaad_Al-Juhaynee_128kbps",
    name: "عبدالله عواد الجهني",
    bitrate: "128kbps",
  },
  45: {
    subfolder: "Salah_Al_Budair_128kbps",
    name: "صلاح البدير",
    bitrate: "128kbps",
  },
  46: {
    subfolder: "Abdullah_Matroud_128kbps",
    name: "عبد الله المطرود",
    bitrate: "128kbps",
  },
  47: {
    subfolder: "Ahmed_Neana_128kbps",
    name: "أحمد نعينع (مرتل)",
    bitrate: "128kbps",
  },
  48: {
    subfolder: "Muhammad_AbdulKareem_128kbps",
    name: "محمد عبد الكريم",
    bitrate: "128kbps",
  },
  49: {
    subfolder: "khalefa_al_tunaiji_64kbps",
    name: "خليفة الطنيجي",
    bitrate: "64kbps",
  },
  50: {
    subfolder: "mahmoud_ali_al_banna_32kbps",
    name: "محمود علي البنا (مجود)",
    bitrate: "32kbps",
  },
  51: {
    subfolder: "Yasser_Ad-Dussary_128kbps",
    name: "ياسر الدوسري",
    bitrate: "128kbps",
  },
  52: {
    subfolder: "Nasser_Alqatami_128kbps",
    name: "ناصر القطامي",
    bitrate: "128kbps",
  },
  53: {
    subfolder: "Ali_Hajjaj_AlSuesy_128kbps",
    name: "علي حجاج السويسي",
    bitrate: "128kbps",
  },
  54: {
    subfolder: "Sahl_Yassin_128kbps",
    name: "سهل زين يس",
    bitrate: "128kbps",
  },
  55: {
    subfolder: "ahmed_ibn_ali_al_ajamy_128kbps",
    name: "احمد العجمي",
    bitrate: "128kbps",
  },
  56: {
    subfolder: "Karim_Mansoori_40kbps",
    name: "كريم منصوري",
    bitrate: "40kbps",
  },
  57: {
    subfolder: "aziz_alili_128kbps",
    name: "عزيز العليلي",
    bitrate: "128kbps",
  },
  58: {
    subfolder: "Yaser_Salamah_128kbps",
    name: "ياسر سلامة",
    bitrate: "128kbps",
  },
  59: {
    subfolder: "Akram_AlAlaqimy_128kbps",
    name: "أكرم العلاقمي",
    bitrate: "128kbps",
  },
  60: {
    subfolder: "Ali_Jaber_64kbps",
    name: "علي جابر",
    bitrate: "64kbps",
  },
  61: {
    subfolder: "Fares_Abbad_64kbps",
    name: "فارس عباد",
    bitrate: "64kbps",
  },
  62: {
    subfolder: "Ayman_Sowaid_64kbps",
    name: "أيمن سويد",
    bitrate: "64kbps",
  },
  63: {
    subfolder: "Husary_Muallim_128kbps",
    name: "محمود خليل الحصري (مُعلِم)",
    bitrate: "128kbps",
  },
  64: {
    subfolder: "Khaalid_Abdullaah_al-Qahtaanee_192kbps",
    name: "خالد عبدالله القحطاني",
    bitrate: "192kbps",
  },
};
