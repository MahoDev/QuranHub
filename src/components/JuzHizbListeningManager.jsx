...
function JuzHizbListeningManager({
	...
	useEffect(() => {
		if (quranRecitations[recitationId] && quranRecitations[recitationId].bitrate) {
			...
			const currentBitrate = bitrate && availableBitrates.includes(bitrate) ? bitrate : availableBitrates[0]; // Use first available if no bitrate set or invalid bitrate
	...
	if (
		quranRecitations &&
		quranRecitations[recitationId] &&
		quranRecitations[recitationId].bitrate
	) {
		const bitrateKeys = Object.keys(quranRecitations[recitationId].bitrate);
		if (bitrateKeys.length > 0) {
			subfolder =
				bitrate == null
					? quranRecitations[recitationId].bitrate[bitrateKeys[0]]
					: quranRecitations[recitationId].bitrate[bitrate] || quranRecitations[recitationId].bitrate[bitrateKeys[0]];
		}
	}
